...
use shpool_protocol::{ConnectHeader, HardcopyReply, HardcopyRequest};

use crate::{protocol, protocol::ClientResult};

pub fn run<P>(session_name: String, plain: bool, socket: P) -> anyhow::Result<()>
where
    P: AsRef<Path>,
{
    ...
    client
        .write_connect_header(ConnectHeader::Hardcopy(HardcopyRequest {
            session_name: session_name.clone(),
            plain,
        }))
        .context("writing hardcopy request")?;

    let reply: HardcopyReply = client.read_reply().context("reading reply")?;

    match reply {
        HardcopyReply::Ok { data } => {
